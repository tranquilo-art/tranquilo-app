-- Add a protected 'delisted' review state, distinct from 'rejected'.
--
...
-- ---------------------------------------------------------------------------
-- A bookmark fallback: a /v/{slug} for a row that exists but
-- isn't LIVE redirects the visitor to the item's own museum page instead of
-- bouncing to the homepage. This is deliberately narrower than "any
-- non-LIVE row": it only fires when the SOURCE INSTITUTION
-- moved or withdrew the object. Something we curated out of our own
-- catalogue (a rejected specimen, a quarantined data-quality gap) isn't owed
-- that treatment just because it was ingested once.
--
-- Neither existing withheld state means that:
--
--   quarantined   the harmonization gate held it back -- OUR data quality
--                 judgment, automated, first-pass
--   rejected      a human reviewer decided it's unfit -- OUR curatorial
--                 judgment, "a specimen that is not art, a record too thin"
--
-- Both are decisions WE made about content we chose to keep ingesting.
-- 'delisted' is a different kind of fact entirely: the institution's own
-- record is gone or moved, verified against their API, and is not a
-- judgment call at all.
--
-- Exactly the same reasoning that split 'rejected' out of 'quarantined' in
-- sql/004 -- these differ in WHO decided and WHY, and conflating them here
-- would blur precisely the distinction that fix depends on.
--
-- Protected exactly like 'reviewed'/'rejected' (see the CASE guards in
-- core.py's Postgres upsert and harmonize_backfill.py's recompute guard,
-- both updated alongside this migration): the gate may not move an item out
-- of it, and a re-ingestion cannot silently revive a delisted row.
