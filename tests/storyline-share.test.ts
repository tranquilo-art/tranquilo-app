...
// buildStorylineIntroPage/buildStorylineChapterPage moved out of
// app.ts into <tranquilo-storyline-mode> (as buildIntroPage/buildChapterPage
// private methods) -- storylineShareUrlFor/storylineIdFromLocation kept
// same-named wrappers in app.ts, since deep-link routing and the
// share-URL shape are still app.ts's job even though their own bodies
// have since moved into src/app/ShareService.ts and src/app/SlugCodec.ts
// respectively.
const storylineModeSource = readFileSync(
  path.join(__dirname, "../src/components/TranquiloStorylineMode.ts"),
  "utf8",
);
// storylineIdFromLocation()'s own body (the /s/{id} path regex) moved out
// of app.ts into src/app/SlugCodec.ts as a class method -- app.ts keeps a
// same-named wrapper, see that file's own header comment.
const slugCodecSource = readFileSync(
  path.join(__dirname, "../src/app/SlugCodec.ts"),
  "utf8",
);
// storylineShareUrlFor()'s own body moved out of app.ts into
// src/app/ShareService.ts as a class method -- app.ts keeps a same-named
// wrapper, see that file's own header comment.
...
describe("app.ts builds and honours storyline share URLs", () => {
  it("shares a /s/ URL, not the cover artwork's /v/ URL", () => {
    ...
    // The actual URL-building body lives in src/app/ShareService.ts now --
  ...
  it("opens a storyline from a /s/{id} cold load", () => {
    ...
    // The actual path regex lives in src/app/SlugCodec.ts now -- see that
