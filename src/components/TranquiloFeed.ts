...
// scratch on every mode switch, so it has to ASK app.ts which filter is
// active on every call rather than being told. See
// src/types/ITranquiloFeedHost.ts for the full contract.
//
// app.ts still creates one shared slideBuilder and one shared
...
// so it lives here as a public property that app.ts's
...
// data-driven and only changes because app.ts called render();
...
// or warn about rapid history writes. Short rather than generous: a
// same-document reload uses whatever URL the browser already committed to
// before this fires, so any value here is a window where a reload lands
// on a stale slide -- this is picked short enough that scrolling has to
// stop and something else has to happen (a click, a reload) for a
// visitor to ever land inside it, while still coalescing every
// per-frame call a continuous scroll produces into one.
const URL_SYNC_DEBOUNCE_MS = 60;
...
export class TranquiloFeed extends HTMLElement {
  ...
  //
  // Public: app.ts's createRecycleWindow() setup needs the same array, by
  // reference.
  ...
  // Decided once per page load, same reasoning as sessionHeroes above:
  // markFeedSeen() flips the localStorage flag the moment the first
  // hero-led render lands, so a bare shouldShowHeroes() call on the "All"
  // render after a category round-trip would already read as a returning
  // visitor and skip heroes entirely, contradicting sessionHeroes' own
  // promise to hold for the rest of this page load.
  private heroesEligible: boolean | null = null;

  private showHeroesThisSession(): boolean {
    if (this.heroesEligible === null) {
      this.heroesEligible = shouldShowHeroes();
    }
    return this.heroesEligible;
  }

  // The "All" feed's own start offset, picked once per page load for the
  // same reason sessionHeroes is: a category round-trip must land back on
  // the same opening, not a fresh Math.random() draw from createPager().
  // Heroes usually cover the first HERO_COUNT slides in production, but
  // nothing guarantees an empty or exhausted pool, so the tail the pager
  // itself controls needs its own stability rather than borrowing theirs.
  private allModeStart: number | null = null;

  private getAllModeStart(): number {
    if (this.allModeStart === null) this.allModeStart = Math.random();
    return this.allModeStart;
  }
  ...
  // predicate, mirroring app.ts's mutually-exclusive activeCategory/
  ...
  render(onSettled?: () => void): void {
    ...
    if (state.concept) {
      ...
      if (state.category === "All") {
        ...
        // storyline. Only for a first-time visitor: showHeroesThisSession()
        // is per-device (localStorage) on its first call, deliberately,
        // since a returning visitor is returning whether or not they
        // closed the tab -- then memoized for this page load, so a later
        // category round-trip doesn't read the flag markFeedSeen() just set.
        if (!state.storylineFilter && this.showHeroesThisSession())
          leadingItems = [];
      } else {
    ...
    if (fixedItems) {
      ...
      if (state.category === "All") pager.start = this.getAllModeStart();
      this.feedPager = pager;
  ...
  // Runs the pending sync immediately rather than waiting out the debounce.
  // Wired to pagehide in app.ts: catches navigating away (a link click,
  // closing the tab) inside the debounce window, so the session history
  // entry left behind points at the slide actually on screen rather than
  // wherever the last flush happened to land. A same-document reload is a
  // different case -- see pagehide's own comment in app.ts.
  flushUrlSync(): void {
