...
async function cleanup() {
  await sql.query("DELETE FROM source_fetch_state WHERE source = $1", [SOURCE]);
  await sql.query(
    "DELETE FROM img_fetch_claims WHERE cache_key LIKE '__verify__%'",
  );
  await sql.query("DELETE FROM img_cache_stats WHERE source = $1", [SOURCE]);
  await sql.query(
    "DELETE FROM img_cache_entries WHERE cache_key LIKE '__verify__%'",
  );
}
...
await sql.query("DELETE FROM img_fetch_claims WHERE cache_key = $1", [KEY]);
{
  const N = 40;
