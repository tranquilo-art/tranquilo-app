...
-- came from and why; this is the live copy. Idempotent via ON CONFLICT.
--
-- "storylines"'s item_ids is '[]'::jsonb, same as it was in
-- src/data/shelves.ts: it has no fixed content of its own, computed
-- instead by src/app.ts from the storylines table's cover_item_id column
-- at feed-init time (see api/items.ts's ?shape=shelves handler).
