...
│ Hosting & Serverless  │ Vercel (Hobby)                 │
│ Package Mgr & Runtime │ Bun (pinned to v1.3.11 in CI)  │
│ Database & Driver     │ Neon Postgres (@neondatabase/serverless) │
│ Ingestion Pipeline    │ Python 3.11+ managed with uv   │
│ Code Quality          │ Biome (JS/TS) + Vitest/Playwright │
└───────────────────────┴────────────────────────────────┘
```
...
* **Raw SQL Queries:** Every application query uses raw SQL via `@neondatabase/serverless` (TypeScript) or `psycopg` (Python).
