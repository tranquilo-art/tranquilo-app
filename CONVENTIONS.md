...
These conventions apply to all TypeScript, and configuration files in the Tranquilo codebase.
...
### TS Stack (Bun + Biome)
* **Package Manager:** Use **Bun** exclusively (`bun install`, `bun run <script>`, `bun test`).
...
1. **Config (Build-Time Input):** Tooling, CI, and dev-time tuning parameters live in `config.toml`.
