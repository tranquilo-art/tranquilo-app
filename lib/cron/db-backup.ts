...
export default async function handler(req: any, res: any) {
  ...
  if (req.query.task === "reshuffle") {
    ...
    try {
      ...
      await client(refreshVocabSql());
      await client(refreshWordsSql());
      const sweptVocab = await client(`${sweepSql("items_vocab")} RETURNING 1`);
      const sweptWords = await client(
