...
- **Ingestion:** a Python pipeline that pulls, licenses-checks, classifies, and merges artwork from each source, managed with [uv](https://github.com/astral-sh/uv) and developed in its own branch/repo.
...
Both run in CI on every push and pull request against `main` ([`.github/workflows/test.yml`](.github/workflows/test.yml)). The ingestion pipeline's own test suite lives with it in its own branch/repo.
...
**Catalogue content:** every ingested artwork is checked against its own source's licensing terms at ingestion time — nothing is added on a blanket "public domain art" assumption.
